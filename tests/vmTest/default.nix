{ pkgs, lib, makeTest, inputs }: let
  # Return a store path with a closure containing everything including
  # derivations and all build dependency outputs, all the way down.
  allDrvOutputs = pkg:
    pkgs.runCommand "allDrvOutputs-${pkg.pname or pkg.name or "unknown"}" { refs = pkgs.writeReferencesToFile pkg.drvPath; } ''
      touch $out
      while read ref; do
        case $ref in
          *.drv)
  ...
  callLocklessFlake = path: inputs: let
    r = {outPath = path;} // ((import (path + "/flake.nix")).outputs (inputs // {self = r;}));
  in r;
  mkNixinateTest = { buildOn, hermetic ? false, ... }: let
        ...
        text = /* nix */ ''
          {
            outputs = { self, nixpkgs }: {
                nixosConfigurations = {
                  nixinatee = (( ((import (nixpkgs + "/nixos/lib/testing-python.nix") { system = "${pkgs.hostPlatform.system}"; }).makeTest)
                  { nodes.baseConfig = { ... }: {}; testScript = "";}).nodes).baseConfig.extendModules {
                  modules = [
    ...
    in makeTest {
      nodes = {
        nixinatee = { ... }: {
          ...
          virtualisation = {
            writableStore = true;
            additionalPaths = lib.concatLists [
